...
int main()
{
    ...
    for(int i = 0;i < threads_num;i++){
        processes[i] = std::thread([i](){
            ...
            char *recv_buf = (char*)malloc(DATA_LEN + 10);
            ...
            for(int iter = 0; iter < ITERS; iter++){
                if(i == 0){
                    ...
                    rdma_conn_object->irecv(recv_buf, DATA_LEN + 10, irecv_req_array + iter);
                }
            }
            if(i == 0)
                rdma_conn_object->wait(isend_req_array + iter - 1);
            else
                rdma_conn_object->wait(irecv_req_array + iter - 1);
