    ...
    for(int i = 0;i < n;i++){
        ...
        if(op == IBV_WC_RECV) {
            ...
            if (ack_ctl_info->type == 0) {
                if(isprintf()) SPP("receive control infomation.\n");
                ...
                mr_pair->send_addr = (uintptr_t)ack_ctl_info->big.send_buffer;
            ...
            //if(isprintf()) SPP("FINISH send a big data...........\n");
        }else{
